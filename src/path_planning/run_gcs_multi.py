...
from .algorithms.gcs_lib import FreespacePolytopes, GraphOfConvexSets
...
def make_obs_for_joint_config_space(obs, n_agents):
    ...
    for obs_i in obs:
        ...
        V = obs_i
        n_columns_to_add = 2 * (n_agents - 1)
        for i in range(n_columns_to_add):
            bounds = 1000 * np.ones((1, V.shape[1]))
            V = np.hstack([np.vstack([V, bounds]), np.vstack([V, -bounds])])
